// Rocket Runner - Peluru Menyesuaikan Arah Gerak (WASD)
#include <iostream>
#include <vector>
#include <conio.h>
#include <windows.h>
...
void render() {
    ...
    if (powerUp) cout << "  [POWER UP: " << sisaPowerUp << "]";
    cout << endl;
}
...
void updateInput() {
    if (_kbhit()) {
        char input = _getch();
        if (input == 'w' && roket.y > 0) { roket.y--; arah = "atas"; }
        else if (input == 's' && roket.y < HEIGHT - 1) { roket.y++; arah = "bawah"; }
        else if (input == 'a' && roket.x > 0) { roket.x--; arah = "kiri"; }
        else if (input == 'd' && roket.x < WIDTH - 1) { roket.x++; arah = "kanan"; }
    }
}
...
void autoTembak() {
    if (arah == "kanan") {
        peluru.push_back({roket.x + 1, roket.y});
        if (powerUp) {
            peluru.push_back({roket.x + 1, roket.y - 1});
            peluru.push_back({roket.x + 1, roket.y + 1});
        }
    } else if (arah == "kiri") {
        peluru.push_back({roket.x - 1, roket.y});
        ...
        peluru.push_back({roket.x, roket.y - 1});
        if (powerUp) {
            peluru.push_back({roket.x - 1, roket.y - 1});
            peluru.push_back({roket.x + 1, roket.y - 1});
        }
...
void updatePeluru() {
    for (auto& p : peluru) {
        if (arah == "kanan") p.x++;
        else if (arah == "kiri") p.x--;
        else if (arah == "atas") p.y--;
        else if (arah == "bawah") p.y++;
    }
    peluru.erase(remove_if(peluru.begin(), peluru.end(), [](Posisi p){ return p.x < 0 || p.x >= WIDTH || p.y < 0 || p.y >= HEIGHT; }), peluru.end());
}

void tembakBossOtomatis() {
    for (auto& p : peluru) {
        if (boss.aktif && p.x == boss.x && p.y == boss.y) {
            boss.hp--;
            p.x = WIDTH + 1;
            if (boss.hp <= 0) {
                boss.aktif = false;
                powerUp = true;
                sisaPowerUp = DURASI_POWERUP;
            }
        }
    }
}
